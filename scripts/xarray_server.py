...
class LazyHealpixInference:
    ...
    def _run_inference(self, times: List[np.datetime64]) -> xr.Dataset:
        ...
        for i in range(0, len(times), batch_size):
            ...
            second_of_day = torch.stack([d["second_of_day"] for d in batch_data]).to(
                device
            )
            ...
            if self.config.seed is None:
                ...
                seeds = [
                    self.config.seed + int(t.astype("datetime64[s]").astype("int"))
    ...
    def create_dataset(
        ...
        lazy_arrays = [
            da.from_delayed(
                self._run_inference(times[i : i + self.config.batch_size]),
                shape=(
