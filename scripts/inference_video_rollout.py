...
from cbottle.video_rollout import VideoRollout, RolloutDiagnostics
...
def write_step_frames(
    ...
    """Write frames from rollout diagnostics, filtering by target end time."""
    frame_mask = diags.new_timestamps[0] < target_end_timestamp
    writer.write_target(
        diags.new_frames[:, :, frame_mask, :],
        diags.coords,
...
def main():
    ...
    if args.sample.sampler == Sampler.fibonacci:
        if args.sample.min_samples > 0:
            sampled_positions = samplers.subsample(
                rollout_tasks, min_samples=args.sample.min_samples
            )
    ...
    logger.info(
        f"\nCompleted {len(my_rollout_tasks)} rollouts on rank {rank} in {total_elapsed/60:.1f} min. "
